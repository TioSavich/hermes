/** <module> Measurement misconception table
 *
 * This table keeps literature-attested measurement misconception
...
misconceptions_measurement_batch_1:(r37528_area_as_perimeter(rect(W,H), Got) :-
    Got is 2*(W+H)).
...
misconceptions_measurement_batch_1:(r37627_cover_by_sum(cover(W-H, TW-_TH), Got) :-
    Got is (W+H) // TW).
...
misconceptions_measurement_batch_1:(r37782_ignore_remainders(planks(N, Each, Target), Got) :-
...
misconceptions_measurement_batch_1:(r38269_remainder_unmeasurable(beam(Len, Unit), Got) :-
    ( Len mod Unit =:= 0
    -> Got is Len // Unit
    ;  Got = 0
    )).
...
misconceptions_measurement_batch_1:(r38583_linear_area_ratio(side(Big)-side(Small), Got) :-
    Got is Big / Small).
...
misconceptions_measurement_batch_1:(r38634_area_scale_linear(map_area(A, scale(S)), Got) :-
    Got is A * S).
...
misconceptions_measurement_batch_1:(r38689_time_base10(hours(H1,M1)-hours(H2,M2), Got) :-
    ( M1 >= M2
...
misconceptions_measurement_batch_1:(r38992_area_as_linear(rect(W,H), Got) :-
    A is W * H,
    Got = cm(A)).
...
misconceptions_measurement_batch_1:(r39392_imperial_collapse(imperial(Ft, Num, Den), Got) :-
    Got is Ft + Num / Den).
...
misconceptions_measurement_batch_1:(r39662_volume_linear_conv(conv(V, m3, cm3), Got) :-
    Got is V * 100).
...
misconceptions_measurement_batch_1:(r40042_linear_cut(cut(L-_W, LL-_WW), Got) :-
    Got is L // LL).
...
misconceptions_measurement_batch_1:(r40493_mean_squared(rect(W,H), Got) :-
    M is (W + H) / 2,
    Got is M * M).
...
misconceptions_measurement_batch_1:(r40641_count_marks(gap(A,B), Got) :-
    D is abs(B - A),
    Got is D + 1).
...
% CONNECTS TO: s(comp_nec(unlicensed(base10_regroup_on_time)))
misconceptions_measurement_batch_2:(r37545_base10_regroup_time(hours(H1,M1)-hours(H2,M2), hours(DH,DM)) :-
    (   M1 >= M2
...
misconceptions_measurement_batch_2:(r37713_half_paste_30(hours(SH,_SM)-hours(AH,AM), hours(GH,GM)) :-
    GH is SH + AH,
    GM = AM).
...
misconceptions_measurement_batch_2:(r38034_subtractive_ratio(N, Got) :-
    Got is N - 1).
...
misconceptions_measurement_batch_2:(r38291_radian_as_length(R, Got) :-
    Got = R).
...
misconceptions_measurement_batch_2:(r38582_linear_tile_count(Side, Got) :-
    Got is (Side * Side) // 8).
...
misconceptions_measurement_batch_2:(r38584_raw_piece_sum([A,B,C], Got) :-
    Got is A + B + C).
...
misconceptions_measurement_batch_2:(r38675_area_as_side(rect(W,_H), Got) :-
    Got = W).
...
misconceptions_measurement_batch_2:(r38694_diagonal_as_unit(A-B, Got) :-
    Got is A + B + 1).
...
misconceptions_measurement_batch_2:(r38993_linear_area_conv(Sqm, Got) :-
    Got is Sqm * 100).
...
misconceptions_measurement_batch_2:(r39003_triangle_abc_product([A,B,C], Got) :-
    Got is A * B * C).
...
misconceptions_measurement_batch_2:(r39524_face_count_volume(box(L,W,H), Got) :-
...
misconceptions_measurement_batch_2:(r39607_decimal_hours_direct(H-DecPart, hours(GH,GM)) :-
    GH = H,
    GM is DecPart * 10).
...
misconceptions_measurement_batch_2:(r39621_area_as_perimeter(rect(W,H), Got) :-
    Got is W * H).
...
misconceptions_measurement_batch_2:(r40010_diameter_ratio(DB-DA, Got) :-
    Got is DB / DA).
...
misconceptions_measurement_batch_2:(r40043_total_area_div(board(LB,WB)-card(LC,WC), Got) :-
    Got is (LB * WB) // (LC * WC)).
...
misconceptions_measurement_batch_2:(r40254_perimeter_two_sides(rect(W,H), Got) :-
    Got is W + H).
...
misconceptions_measurement_batch_2:(r40638_first_foot_uncounted(N, Got) :-
    Got is N - 1).
...
misconceptions_measurement_batch_2:(r38879_always_common_unit(segments(side, diagonal), true)).
...
misconceptions_measurement_batch_2:(r38965_visible_faces_volume(cube(2,2,2), 12)).
...
misconceptions_measurement_batch_2:(r39551_decimal_time_minutes(decimal_time(23,78), invalid_minutes)).
...
misconceptions_measurement_batch_2:(r40241_area_halved_for_side(area_square(A), Side) :-
    Side is A / 2).
...
misconceptions_measurement_batch_2:(r37875_triangle_sum_scales(triangle(large), 770)).
...
misconceptions_measurement_batch_2:(r38063_same_perimeter_recip(rect(10,5), 30)).
...
misconceptions_measurement_batch_2:(r39012_circumference_overestimate(radius_increase(1.8), much_more)).
...
misconceptions_measurement_batch_2:(r39106_contour_area_not_equal(areas_between_parallel_curves, not_equal)).
...
misconceptions_measurement_batch_2:(r39141_area_requires_congruence(noncongruent_equal_area, impossible)).
...
misconceptions_measurement_batch_2:(r39143_additive_area_balance(triangles((1,9),(3,7)), equal)).
...
misconceptions_measurement_batch_2:(r39265_single_equator(sphere, one_equator)).
...
misconceptions_measurement_batch_2:(r39266_spherical_plane_sum(spherical_triangle, 180)).
...
misconceptions_measurement_batch_2:(r39516_additive_similarity(rect(3,5)-rect(5,7), true)).
...
misconceptions_measurement_batch_2:(r39585_area_scales_linearly(scale2d(K), K)).
...
misconceptions_measurement_batch_2:(r40011_volume_scales_linearly(edge_scale(K), K)).
...
misconceptions_measurement_batch_2:(r40244_pythagorean_area(legs(A,B), Area) :-
    Area is A * B / 2).
...
misconceptions_measurement_batch_2:(r40261_triangle_area_no_half(triangle(B,H), Area) :-
    Area is B * H).
...
misconceptions_measurement_batch_2:(r40265_rectangle_three_triangles(rectangle, 540)).
