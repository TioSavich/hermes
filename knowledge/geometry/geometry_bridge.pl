...
% knowledge/geometry/schema.pl owns the canonical load chain used here and by the
...
%
%   swipl -l paths.pl -s knowledge/geometry/geometry_bridge.pl
%
...
% paths.pl (plain `swipl -s knowledge/geometry/geometry_bridge.pl`), consult it from
% the parent directory of this file.
:- prolog_load_context(directory, GeomDir),
   (   user:file_search_path(geometry, _)
   ->  true
   ;   file_directory_name(GeomDir, KnowledgeDir),
       file_directory_name(KnowledgeDir, RepoRoot),
       directory_file_path(RepoRoot, 'paths.pl', PathsFile),
       (   exists_file(PathsFile)
       ->  consult(PathsFile)
       ;   throw(error(existence_error(source_sink, PathsFile),
                       context(geometry_bridge, 'paths.pl not found beside knowledge/geometry/')))
       )
   ).
