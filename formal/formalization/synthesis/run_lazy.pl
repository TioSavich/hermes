% Verify base-invariance of cost-bounded synthesis.
%
% Runs discovery for the structurally-matched make-base subtraction targets
% under bases 5, 7, 10, and 12, prints the cost-optimal productive program
% in each, and reports whether the program shapes match modulo the base
% parameter.
%
% Expected outcome: the same canonical four-step make-base structure
% emerges under every base. Strategic invariance (Proposition 1 in
% ALGEBRAIC.tex) is then observed empirically as a corollary of structural
% cost.
%
%   swipl -q -l paths.pl -l formal/formalization/synthesis/run_lazy.pl -g main -t halt

:- use_module(formalization(synthesis/synth_lazy)).

main :-
    Bases = [
        subtract_near_base_b5,
        subtract_near_base_b7,
        subtract_near_base_b10,
        subtract_near_base_b12
    ],
    format("~n========================================~n"),
    format("Base-invariance verification~n"),
    format("========================================~n"),
    forall(member(T, Bases), run_one(T)),
    format("~n========================================~n"),
    format("Verification complete.~n"),
    format("If the program shapes above are identical modulo~n"),
    format("the base parameter, Proposition 1 of ALGEBRAIC.tex~n"),
    format("(strategic invariance under base shift) holds for~n"),
    format("this cost model.~n"),
    format("========================================~n").
