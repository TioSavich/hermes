#!/usr/bin/env bash
# Re-check the finite emergent-hyperedge search without network access.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="${BIGRED_REPO_ROOT:-$(cd "$HERE/../../.." && pwd)}"
OUTPUT_ROOT="${BIGRED_OUTPUT_ROOT:-$REPO_ROOT/.bigred-output/hyperedges}"
LIMIT=0
...
BIGRED_OUTPUT_DIR="$OUTPUT_ROOT" BIGRED_LIMIT="$LIMIT" \
  swipl -q -l paths.pl -s scripts/bigred/suite/suite_batch.pl -g 'suite_batch:main(hyperedges)' -t halt
