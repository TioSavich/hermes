#!/usr/bin/env bash
# Materialize the recorded curriculum-and-strategy graph without simulation.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="${BIGRED_REPO_ROOT:-$(cd "$HERE/../../.." && pwd)}"
OUTPUT_ROOT="${BIGRED_OUTPUT_ROOT:-$REPO_ROOT/.bigred-output/learner-paths}"
LIMIT=0
...
BIGRED_OUTPUT_DIR="$OUTPUT_ROOT" BIGRED_LIMIT="$LIMIT" \
  swipl -q -l paths.pl -s scripts/bigred/suite/suite_batch.pl -g 'suite_batch:main(learner_paths)' -t halt
touch "$OUTPUT_ROOT/COMPLETE"
printf 'Learner-path graph complete: %s\n' "$OUTPUT_ROOT"
