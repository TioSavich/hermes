...
cw_rule((domain_context_mapping_witness(A,B,_14156{context:B,domain:A,kind:finite_domain_context_mapping,mapping_table:[n-natural_numbers,z-integers,q-rationals],source_file:'learner/axioms_domains.pl'}):-domain_context_pair(A,B),catch(sequent_engine:domain_to_context(A,B),_,fail))).
