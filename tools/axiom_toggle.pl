 ...
 *     rhythm table in `pml/semantic_axioms.pl`, which feeds
 ...
 * axiom: `pml/semantic_axioms.pl` labels
 ...
 * (`tools/axiom_pack_audit.pl`) classifies non-termination by timeout:
 ...
 * `tools/tests/test_axiom_toggle.pl` shows that call timing out with the
